...
local function display_notification(message)
	wezterm.log_info(tostring(message))
...
local function get_workspace_for_input_selector()
	...
	if rg == nil then
		...
		for line in rg:lines() do
			local filename = string.gsub(line, ".json", "")
			table.insert(ws, { label = filename, id = filename })
		end
		rg:close()
	end
	return ws
...
function session_manager.load_state(window, pane, id, label)
	...
	local function load_state_internal(window, pane, id, label)
		...
		local workspace_data = load_from_json_file(file_path)
		if not workspace_data then
			window:toast_notification("WezTerm", "Workspace state file not found for workspace: " .. id, nil, 4000)
			return
		end

		if recreate_workspace(window, workspace_data) then
			window:toast_notification("WezTerm", "Workspace state loaded for workspace: " .. id, nil, 4000)
		else
			window:toast_notification("WezTerm", "Workspace state loading failed for workspace: " .. id, nil, 4000)
		end
	end
	local choices = get_workspace_for_input_selector()
	window:perform_action(
		wezterm.action.InputSelector({
			title = "test",
			action = wezterm.action_callback(load_state_internal),
			choices = choices,
		}),
		pane
	)
